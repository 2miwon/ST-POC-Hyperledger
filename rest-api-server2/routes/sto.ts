...
router.post('/mint', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (findUser) {
            ...
            const createToken = await invokeChaincode('Mint', [userId, stId.toString(), offerQty]);
    ...
    try {
        ...
        if (findBuyUser && findSellUser && findST && findBuyUser.wallet_addr && findSellUser.wallet_addr) {
            ...
            if (enoughFiat && enoughFiat.fiat_balance && enoughST && enoughST.relationships) {
                ...
                const transferToken = await invokeChaincode('ProcessTransferBatch', transferBatch);
